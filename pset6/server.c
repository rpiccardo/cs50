...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            //loop through entire request-line
            ...
            *respond with 501
            */
                //declare pointer to beginning of request-target
                char* req = sp + 1;
                if(*req != '/')
                {
                    error(501);
                ...
                /*
                *extract request-target from line
                */
                
                    //declare pointer to space2
                    char* sp2 = strchr(req,' ');                   
                    //lenth of request
                    int requestlen = sp2 - req;
                    //buffer
                    char request[requestlen + 1];
                    strncpy(request, req, requestlen);
                    request[requestlen] = '\0';
                    
                /*
                *if request target contains a ", 
                *respond to the browser with 400
                */
                 
                    //iterate through request buffer
                ...
                /*
                *extract HTTP-version from line
                */
                
                        //declare pointer to \r
                        char* end = strchr(sp2,'\r');                   
                        //lenth of HTTP
                        int httplen = end - (sp2 + 1);
                        //buffer
                        char http[httplen + 1];
                        strncpy(http, sp2 + 1, httplen);
                        http[httplen] = '\0';
                        
                /*
                * if HTTP-version is not HTTP/1.1, then error 505
                */
                 ...
                 */
                        ...
                        //buffer
                        char abs[abslen + 1];
                        strncpy(abs, req, abslen);
                        abs[abslen] = '\0';
                        
                /*
                *if absolute-path does not contain a '.', 
                *respond to the browser with 501
                */
                        int dot = 0;
                        
                        //iterate through abs buffer
...
const char* lookup(const char* extension)
{
    ...
    if(extension != NULL)
    {
        //returns text/css if extension is css
        if((strcasecmp(extension, "css")) == 0)
        {
            return "text/css";
        }
        //returns text/html is extension is html
        if((strcasecmp(extension, "html") == 0))
        {
            return "text/html";
        }
    
        //image/gif if extension is gif
        if((strcasecmp(extension, "gif") == 0))
        {
            return "image/gif";
        }
        
        //image/x-icon if extension is ico
        if((strcasecmp(extension, "ico") == 0))
        {
            return "image/x-icon";
        }
        
        //image/jpeg if extension is jpg
        if((strcasecmp(extension, "jpg") == 0))
        {
            return "image/jpeg";
        }
        
        //image/png if extension is png
        if((strcasecmp(extension, "png") == 0))
        {
            return "image/png";
        }
        
        //text/javascript if extension is js
